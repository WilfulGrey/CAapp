...
| `project 3/app/admin/wachstum/page.tsx` + `app/api/admin/wachstum/route.ts` + `app/api/admin/wachstum/kosten/route.ts` + `lib/wachstum.ts` | Admin-Seite „Wachstum“ unter „Mehr“ (Registry #71): Potenzialentwicklung im laufenden Monat (fest gebucht + Suchende mit fertigem Profil), Kunden im Einsatz je Tag, Anfragen und fertige Profile je Tag im Wochenschnitt (eigen/eingekauft). Rechnen pure in `lib/wachstum.ts` (Test `src/__tests__/wachstum.test.ts`), Route mit Service-Key hinter dem Admin-Cookie, an den Browser nur Summen. Karte „Wo wir stehen“ (Registry #73): Provision je Kunde minus variable Kosten, Werbung (automatisch: `ads_kosten_tag` + eingekaufte Anfragen × `PORTAL_PREISE`) und Gemeinkosten je Monat; Eingabe per `PUT /api/admin/wachstum/kosten` in Tabelle `wachstum_monat` (RLS ohne Policies, nur service_role). Das Dashboard `/admin` bleibt unverändert |
...
`angaben-labels.ts` (Registry #55), `oaiq.ts`, `pflegena.ts` (Registry #59), `zaehler.ts` (Registry #63), `wachstum.ts` (Registry #71, #73), `telefon.ts` (Telefon im Kontakt-Schritt: nur Ziffern/+/Trennzeichen, 8–15 Ziffern; Server bleibt bei ≥6), `rueckmeldung.ts` (Registry #72).
