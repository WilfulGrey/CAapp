...
| `project 3/lib/preis-zuerst.ts` + `components/calculator/PreisSeite.tsx` | Preis zuerst (Registry #77): Ablaufwahl (`?ablauf=preis\|alt`, sessionStorage), Wortlaut der Preisseite (= Kostenkarte des Portals), Texte des Kontakts hinter dem Preis, kurze Warteseite; die Seite selbst als eigene Komponente |
| `project 3/lib/kontakt-stufen.ts` + `app/api/lead-telefon/route.ts` | Kontakt in drei Schritten (Registry #76): Variantenwahl (`?kontakt=stufen\|alt`, sessionStorage), Texte der Teilschritte; Route trägt die Rückrufnummer per Token nach (`leads.telefon`, Event `telefon_nachgetragen`, Team-Mail) |
| `project 3/lib/wechsel.ts` + `app/wechsel/page.tsx` | Landingpage für Anzeigen an Familien mit bestehender 24-Stunden-Kraft (Registry #81): Texte pure (Garantie nur aus `GARANTIE`), Seite host-gated + noindex, Rechner mit festem Ablauf `preis`, Zähler-Variante `wechsel`, Lead-Ereignis `kontakt_variante.seite` |
...
| 80 | **Ablauf-Test: kein Preis + Kontakt in drei Schritten (A) gegen Preis zuerst + eine Kontaktseite (B), 50/50 je Sitzung** (2026-09-18/19, Martin: „Preis zeigen wir nicht an, den Kontakt dafür in drei Schritten. Als A/B-Test mit der anderen Variante, wir zeigen den Preis vorher an“ — nach einem Missverständnis: der erste Bau würfelte den Kontakt HINTER dem Preis). Stand nach 46 h Preis zuerst für alle: 29 Preisseiten-Besucher → 13 Knopf → 3 Personen Anfrage (10 % statt 28 % am alten Kontaktschritt; Google-Sitzungen → Anfragen 3,5 % statt 6,4 %). **Bau:** `lib/preis-zuerst.ts` `ABLAUF_TEST = { aktiv, anteilPreis }`, `ablaufVariante(search, storage, zufall)` zieht das Los einmal und merkt es in `prim_ablauf_variante` (klebt je Sitzung; ohne Storage `preis`, damit es nicht bei jedem Aufruf neu fällt); `?ablauf=alt|preis` erzwingt. `lib/kontakt-stufen.ts` `kontaktStandard(ablauf)`: `alt` → `stufen` (Registry #76: Name → E-Mail → Telefon, Lead nach der E-Mail, Warteseite 10,7 s, Kopf „Ihr Preis ist berechnet“), `preis` → `alt` (= `KontaktSeite.tsx`); `?kontakt=stufen|seite` erzwingt die Form unabhängig vom Ablauf; die Karten-Seite (`?kraefte=1`) bleibt beim alten Formular ohne Los. Das Los fällt im Mount-Effekt von `MultiStepForm` (nie im Render — React #418/#423), die Fragen 1–8 laufen also schon unter dem Los. **Zähler:** Arm A = Variante `stufen`, Arm B = `preis` (unverändert, keine neuen Werte); je Anfrage steht der Arm im Lead-Ereignis `kontakt_variante.ablauf` (`alt` = A, `preis` = B; dazu `variante`, `telefon_dabei`). **Messen:** `preis_zuerst.py` Abschnitt „Ablauf-Test“ (`TEST_SEIT` beim Merge auf die Merge-Stunde setzen): Starts → Schritt 9 → Anfragen je Arm, alle/Google, A mit Name → E-Mail → Anfrage → Telefon → Nummer/ohne, B mit Preisseite → Kontakt geöffnet → Anfrage; Anfragen/Profile/Nummer je Arm aus `kontakt_variante`. Kennzahl = Anfragen je Start (gleicher Nenner, weil das Los vor Frage 1 fällt), Aussage ab 60 Schritt-9-Besuchern je Arm (ca. 8 Tage bei 22 Starts/Tag). **Regeln:** Notbremse für B bleibt (< 4 % Google-Anfragen je Sitzung nach 7 Tagen); Entscheidung nach Anfragen je Start UND fertigen Profilen UND Nummern (A legt den Lead schon nach der E-Mail an — Anfragen ohne Nummer sind kein Gewinn). Aus: `ABLAUF_TEST.aktiv = false` (alle `preis`) — oder `anteilPreis: 0` (alle A). Zweig `feat/kontakt-ab-test`, PR #734, Merge = Prod nur mit Martins GO. |
| 81 | **Landingpage `/wechsel` für Familien, die schon eine 24-Stunden-Kraft haben** (2026-09-18/19, Martin: „bei der Kampagne geht es doch eher darum, unzufrieden mit ihrer Pflegekraft oder Agentur oder ist das zu teuer? … eigene Landingpage vermutlich“). Suchvolumen-Befund: Wechsel-Begriffe („Agentur wechseln“, „kündigen“, „zu teuer“) haben bei Google keine messbare Nachfrage, im Konto in 90 Tagen 0 Suchbegriffe — die Zielgruppe erreicht man über Agenturnamen (seit 07.09. bewusst negiert), „ohne Agentur/privat“ (bester CPA im Konto, seit 26.08. negiert) und „Agentur gesucht“; die Wechsel-Botschaft steht in Anzeige und Seite. **Bau:** `app/wechsel/page.tsx` (Server-Komponente, host-gated + `noindex` wie `/kosten-berechnen`, Hero-Raster der Startseite mit beiden Siegeln, Texte aus `lib/wechsel.ts`), Abschnitte „Was sich ändert, wenn Sie wechseln“ (Preis mit `GARANTIE.zusage`+`ablauf`, Bindung, Pflegekraft mit Ersatz „in der Regel innerhalb von drei Tagen“, Ansprechpartner), „So läuft der Wechsel“ (4 Schritte), Marta + Garantie-Karte, „Fragen vor dem Wechsel“ (5), Testsieger, FinalCTA. Kein Wettbewerbername, kein „günstiger als“, kein „verdient mehr“ (Mail-Wortlaut ohne Zahl, zieht Bewerberinnen). **Rechner auf der Seite:** `MultiStepForm` bekommt `knopf` („Preis in 2 Minuten vergleichen →“), `ablaufFest="preis"` (immer Preis zuerst, kein Los, nicht gemerkt) und `zaehlerVariante="wechsel"` (eigene Zähler-Variante in `ZAEHLER_VARIANTEN`, damit die Arme des Ablauf-Tests sauber bleiben); der Lead-Body trägt `seite` (Pfad), die Route schreibt es sanitisiert (`/^\/[a-z0-9\-\/]{0,40}$/i`) ins Ereignis `kontakt_variante` — `preis_zuerst.py` lässt Anfragen mit `seite = /wechsel` aus den Armen. Traffic-Trennung von selbst über `analytics_sessions.landing_page`. Test `src/__tests__/wechsel.test.ts` (Garantie-Wortlaut, verbotene Wörter, Zahlen nur belegte). Kampagnenplan und Anzeigentexte: Memory `kampagne-direkt-statt-agentur`; Anlage der Kampagne nur nach 21 Uhr mit Martins GO. Zweig `feat/kontakt-ab-test` (auf #734 gestapelt), Merge = Prod nur mit Martins GO. |
