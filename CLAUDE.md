...
| 64 | **Clarity bekam den Magic-Link-Token als Nutzer-ID** (2026-09-11, Martin; Befund aus der PostHog-Doku, PR #697). `identifyClarity(lead.token)` im Portal (`CustomerPortalPage`) und auf der Kalkulationsseite des Rechners setzte `clarity('set','userId', token)` — damit lag der Zugang zum Kundenkonto (`kundenportal…/?token=…`) als Klartext-Tag bei Microsoft. | `src/lib/clarity.ts`, `project 3/lib/clarity.ts`, `src/pages/CustomerPortalPage.tsx`, `project 3/app/kalkulation/[leadId]/page.tsx`, `src/__tests__/clarity.test.ts`, `src/__tests__/integration/portal.test.tsx` (+1), `test/fixtures/mamamia-mocks.ts` (Fixture-`id` jetzt UUID-förmig) | Beide Seiten taggen mit der **Lead-ID** (`leads.id`, UUID) — dieselbe Zeile, also bleibt die Verknüpfung Rechner↔Portal. Zweite Schicht im Helfer: nur UUID-förmige Werte gehen durch; der Token (32 Zeichen alphanumerisch, `generateToken`) kann nicht mehr hineinrutschen, auch wenn ein Aufrufer die Felder verwechselt. Portal-Test prüft den Aufrufort (Clarity erhält `lead.id`, nie den Token) und schlägt mit dem alten Aufruf fehl. **Nicht behoben (offen):** Clarity zeichnet die Seiten-URL auf, und die trägt im Portal weiter `?token=…` — der Token erreicht Clarity also noch über die Adresse. **Regel:** An Dritte (Analyse, Aufzeichnung, Pixel) geht als Kennung nur die Lead-ID, nie Token, Name, E-Mail oder Telefon. |
