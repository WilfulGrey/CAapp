...
| `project 3/lib/marta-karte.ts` + `lib/bewertungen-stand.ts` (+ Kopien `send-scheduled-emails/martaKarte.ts`, `bewertungenStand.ts`) | **Martas Signaturkarte — EINE Vorlage** für alle Mails mit Karte (Martin 17.09.2026; vorher 7 Code-Kopien): `martaKarteHtml({ fuer: 'kunde', bewertung, siteUrl, presseLogos, abstandUnten? })`. „✆ Anrufen“ + „WhatsApp“ in einer Zeile, darunter IN der Karte „★★★★★ 4,9 von 5 · 126 Bewertungen →“ (→ primundus.de/erfahrungen), Faktenzeile rechts „Bestpreisgarantie, keine Vermittlungsgebühr“. `fuer: 'vermittler'` = ohne Sterne und mit „Persönlicher Ansprechpartner“ (Kunden-Konditionen neben dem Provisionsblock verboten, #59). Handy-Regeln (#702/#78 + Knöpfe umbrechen) in `MARTA_KARTE_MOBIL_CSS`, jede Mail-Shell bindet sie in ihre Media-Query ≤480 px ein. Stand live aus `GET https://primundus.de/api/bewertungen-stand`, Timeout 2 s, Ersatzwert `{ schnitt: '4,9', anzahl: 126 }` bei Fehler/ungültiger Antwort (`schnitt` /^\d,\d$/ und 1,0–5,0, `anzahl` ganzzahlig ≥ 1). Kostenrechner: `holeBewertungsStand()` mit Prozess-Cache (1 h, Fehlschlag 10 min), Vorlagen in `lib/email.ts` bekommen den Stand als Pflichtparameter. Edge Function: `index.ts` lädt einmal pro Aufruf in `bewertungsStand`, `buildMartaSig(site, "vermittler")` für Vermittler-Mails. Ohne Karte bleiben die Bewertungsanfrage (eigene Signatur) und die Textfassungen. Statische Vorlagen `mail-templates/15`, `19` = gerenderte Vermittler-Karte. Tests: `src/__tests__/martaKarte.test.ts` (inkl. Kopienvergleich), `bewertungenStand.test.ts`, `mails/martaKarteInMails.test.ts` (Quellen-Wächter: keine eingebettete Karte, Shells mit CSS), Deno `_tests/martaKarte.test.ts`, `_tests/bewertungenStand.test.ts` |
