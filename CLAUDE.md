...
| 66 | **Brak środków na koncie Anthropic odrzucał anfrage NA ZAWSZE** (2026-09-11, prod: uid 17650 Ute Schüle i uid 17695 Petra Keßler od Pflegeny). `dauerhaft = res.status === 400` (`route.ts`) — a Anthropic melduje puste konto jako **400 `invalid_request_error`** z komunikatem „Your credit balance is too low to access the Anthropic API". Obie maile wpadły w `abgelehnt` po PIERWSZEJ próbie: shell-lead `manuell_pruefen` + team-mail „Vermittler-Anfrage unbeantwortet", i nic ich już nie ruszało — odpowiedzi dla partnera trzeba było 12.09 wyjmować ręcznie (`update portal_mail_log set status='offen' where postfach='pflegena.com' and uid=…`). To ta sama klasa co „401 po rotacji klucza", którą kod od początku traktuje jako przejściową — tylko przyjeżdża pod kodem zarezerwowanym dla werdyktu o TREŚCI maila. | `project 3/lib/pflegena.ts` (`modellFehlerArt`), `app/api/portal-abholen/route.ts` (`ModellErgebnis`, `PostErgebnis.guthaben`, `ohneZaehler`), `src/__tests__/pflegena.test.ts` (+4), docs/portal-leads.md | Trzecie lager **`guthaben`**: `offen` **i bez licznika** (`versuche` nie rośnie), bo wywołanie odrzucone przed inferencją nic nie kosztuje — sufit `MAX_VERSUCHE=5` broni PŁATNYCH prób, a przy awarii trwającej godziny zjadłby maila po pięciu minutach i wynik byłby ten sam co przed fixem. Powtórka bez załącznika też odpada (załącznik nie był problemem, a jej hinweis wsiąkłby w opis zlecenia). **Jedyne miejsce, w którym klasyfikujemy po TREŚCI błędu, nie po strukturze** — innego sygnału nie ma; `billing_error` (403) łapiemy jako strukturalną formę tego samego. **Cena decyzji:** dopóki brak środków, mail wisi `offen`, bieg zwraca 500, kolejka vermittlera stoi (rusza sama po doładowaniu) — i **nie ma już team-maila** o tym przypadku, więc trwale puste konto widać tylko w logach. **Reguła:** kod HTTP nie mówi, CZYJ to problem — 400 bywa werdyktem o treści ORAZ o stanie konta; zanim uznasz błąd za trwały, sprawdź, czy ponowienie w ogóle coś kosztuje.
