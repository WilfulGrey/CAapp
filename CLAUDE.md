...
| 72 | **„Aktuell nicht" / „Doch nicht relevant" in der Abschiedsmail stoppten nichts** (2026-09-14, Martin: „wenn kunde antwortet, dass er gerade kein interesse hat, würde ich nicht mehr mails senden und nachfassen stoppen … nichts im SA-Portal"). Die zwei Knöpfe von nachfass_3 waren `mailto:` an info@ (Reply-To aller Kundenmails). Status `nicht_interessiert` stoppt zwar alles (Nachfass, Erinnerungen, warum_primundus, neue Kräfte; detect-caregiver-events scannt den Lead nicht mehr, also keine Bewerbungsmails), aber den setzte nur ein Mensch im Admin. | `project 3/lib/kein-interesse.ts` NEU (pure), `project 3/app/api/kein-interesse/route.ts` NEU, `project 3/app/kein-interesse/{page,layout}.tsx` NEU, `next.config.js` (noindex), admin `leads/[id]/page.tsx` (Label), `send-scheduled-emails/{kette.ts,index.ts}`, Tests: `src/__tests__/keinInteresse.test.ts` (+10), `_tests/kette.test.ts` (+1) | Knöpfe → `/kein-interesse?token=…&grund=aktuell-nicht\|nicht-relevant` (`keinInteresseLink`, ohne Token bleibt es beim mailto). Die Seite fragt „Keine E-Mails mehr?" und schickt erst nach dem Klick auf „Ja, keine E-Mails mehr" ein POST: **nie beim blossen Aufruf**, weil Mail-Scanner (Outlook Safe Links, Gmail) Links selbst öffnen und sonst interessierten Kunden die Mails abstellen würden. Route wirkt wie der Admin-Knopf: `status = nicht_interessiert` + alle `pending`-Mails canceln + Event `kunde_kein_interesse` {grund, quelle: nachfass_3, status_vorher, status_gesetzt, mails_gestoppt} + Team-Mail an info@ (`teamMailKeinInteresse`, `skipBcc`). Nur offene Anfragen (`angebot_requested`/`info_requested`/`manuell_pruefen`) werden umgestellt; gebuchte Kunden und Folge-Einsätze bleiben unangetastet (der Link liegt Wochen im Postfach), das Team bekommt dann nur die Info. Idempotent über das Event, unbekannter Token ⇒ 200 ohne Wirkung (keine Enumeration), Service-Key Pflicht (kein Anon-Fallback). **Deploy-Reihenfolge umgekehrt:** erst Merge (Seite + Route live auf Render), DANN die Mail-Funktion auf Prod, sonst verlinkt eine frische Abschiedsmail eine 404. Knopf-Texte und Seiten-Labels werden per Test gegeneinander geprüft (`Gleichlauf mit der Mail-Funktion`). Freie Antworten an info@ bleiben beim Team (Admin-Knopf); automatisch erkennen hiesse Kundenantworten durch ein Sprachmodell zu schicken. |
...
`angaben-labels.ts` (Registry #55), `oaiq.ts`, `pflegena.ts` (Registry #59), `zaehler.ts` (Registry #63), `wachstum.ts` (Registry #71), `telefon.ts` (Telefon im Kontakt-Schritt: nur Ziffern/+/Trennzeichen, 8–15 Ziffern; Server bleibt bei ≥6), `kein-interesse.ts` (Registry #72).
