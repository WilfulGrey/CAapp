...
| 83 | **Provision beim Anlegen von Kunde und Job: 0 statt 10 €** (2026-09-24, Michał: „jak zakładamy joba klienta, to robimy tam prowizję SA 10 euro. Potrzebujemy, żeby było to 0 euro"). Onboard schrieb seit 11.05. (vorher 300) den Platzhalter 10 in ZWEI Felder: `Customer.commission_agent_salary` (StoreCustomer) und `JobOffer.salary_commission` (StoreJobOffer). MM rechnet `care_budget = monthly_salary − commission_agent_salary` — neue Kunden hatten deshalb `care_budget` = Brutto − 10. | `supabase/functions/onboard-to-mamamia/{mappers,onboard,types}.ts`, `_tests/{mappers,onboard}.test.ts`, docs/customer-portal-flow.md §5 ⑤ (dort stand noch 300) | Beide Felder = 0. **Live auf beta belegt** (Staging-Lead → Onboard mit neuem Code, Customer 10040 / JobOffer 33569): `commission_agent_salary=0`, `care_budget=monthly_salary=3200`, `salary_commission=0`. Validierungssonde (temporäre Edge Fn, `service_agency_id` absichtlich ungültig ⇒ nichts angelegt): die MM-API hat für beide Felder **gar keine Regel** — sogar −5 geht durch; rebing meldet alle Verstöße auf einmal (StoreCustomer: 4 Fehler gleichzeitig). Der alte Kommentar „panel rejects 0" meinte also das Panel-FORMULAR, nicht die API — wer so einen Kunden/Job im Panel bearbeitet, muss dort ggf. eine Provision eintragen. Bestehende Kunden/Jobs behalten 10 (kein Backfill; Datenkorrektur in MM macht das MM-Team). `ServiceAgency.commission` (beta 500) ist ein anderes Feld und bleibt unberührt. |
