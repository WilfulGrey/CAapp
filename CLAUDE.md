...
| `project 3/lib/bewertungen-stand.ts` (+ Kopie `supabase/functions/send-scheduled-emails/bewertungenStand.ts`) | Bewertungszeile unter Martas Karte in JEDER Kundenmail mit Karte (Martin 17.09.2026): „★★★★★ 4,9 von 5 aus 126 Bewertungen · Erfahrungen lesen →“ → primundus.de/erfahrungen. Stand live aus `GET https://primundus.de/api/bewertungen-stand` (Website-Repo), Timeout 2 s, Ersatzwert `{ schnitt: '4,9', anzahl: 126 }` bei Fehler/ungültiger Antwort (`schnitt` /^\d,\d$/ und 1,0–5,0, `anzahl` ganzzahlig ≥ 1). Kostenrechner: `holeBewertungsStand()` mit Prozess-Cache (1 h, Fehlschlag 10 min), Vorlagen in `lib/email.ts` bekommen den Stand als Pflichtparameter. Edge Function: `index.ts` lädt einmal pro Aufruf in die Modulvariable `bewertungsStand`, `buildMartaSig(site, { mitBewertung: false })` für Vermittler-Mails. Bewusst OHNE Zeile: Vermittler-Mails, Bewertungsanfrage, Textfassungen. Tests: `src/__tests__/bewertungenStand.test.ts` (inkl. Kopienvergleich), `src/__tests__/mails/bewertungsZeileUnterMartaKarte.test.ts` (Quellen-Wächter), Deno `_tests/bewertungenStand.test.ts` |
