...
| `project 3/app/admin/page.tsx` + `app/api/admin/wachstum/route.ts` + `lib/wachstum.ts` | Startseite des Admins „Wachstum“ (Registry #69): Kunden im Einsatz je Tag, Anfragen und fertige Profile je Tag im Wochenschnitt (eigen/eingekauft), Kacheln 7/30 Tage. Rechnen pure in `lib/wachstum.ts` (Test `src/__tests__/wachstum.test.ts`), Route mit Service-Key hinter dem Admin-Cookie, an den Browser nur Summen |
