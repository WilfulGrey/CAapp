...
| `upload-offline-conversions/` | **Cron täglich 06:20 UTC** — lädt ZWEI Offline-Conversions zu Google Ads (beide secondary): „Qualifizierte Leads" (erstes `patient_data_saved` → Aktion `7720728390`, Status `offline_conversion_uploads`) und seit 21.08. „Kunde gebucht" (erste `lead_application_acceptances`-Zeile je Lead, fix 400 € → Aktion `7728914324`, Status `offline_booking_uploads`, transactionId `booking-<lead_id>`). Lead trägt gclid/wbraid/gbraid; ohne Google-Secrets (Staging) inert. Doku: docs/google-ads-tracking.md |
