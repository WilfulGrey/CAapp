...
function kontakt(){
  ...
  go.onclick=async()=>{
    ...
    /* Derselbe Weg wie beim Formular: /api/pria/lead rechnet den Preis und
       ...
       Magiclink entstehen dort. Zurück kommt die portalUrl. */
    let portalUrl=null, gescheitert=false;
    try{
      const res=await fetch('/api/pria/lead',{method:'POST',
        ...
        body:JSON.stringify({sid:SID,name:felder[0].value.trim(),email:felder[1].value.trim(),
          telefon:felder[2].value.trim(),antworten:Object.assign({},antwort)})});
    ...
    if(gescheitert){
      ...
      await sagen('Da ist beim Anlegen etwas schiefgegangen — Ihre Angaben sind bei uns, aber der '+
                  'Zugang steht noch nicht. Ilka meldet sich bei Ihnen. Wenn es eilt: <b>089 200 000 830</b>.');
    ...
    if(portalUrl){
      setChips([{t:'→ Ihr Kundenportal öffnen',stil:'stark',f:()=>{location.assign(portalUrl);}}]);
      // Wie auf der Ergebnisseite des Formulars: nach dem Countdown von selbst.
      setTimeout(()=>{ if(!ruhig) location.assign(portalUrl); }, 1600);
    }else{
      setChips([{t:'→ Kundenportal (Link folgt per E-Mail)',stil:'soft',f:()=>{}}]);
    }
  };
}
