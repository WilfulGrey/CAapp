...
/* Voll-Chat-Modus (Landingpage /sofortangebot): die Seite setzt data-pria-voll,
...
/* Welche Test-Variante der Besucher sieht. Die Weiche (middleware.ts)
   liefert A/B/C alle unter „/" aus — der Pfad allein sagt also nichts.
   Dasselbe Vorgehen wie in lib/analytics.ts. */
function variantenSeite(){
  const echt=location.pathname||'/';
  if(echt!=='/') return echt;
  const v=/(?:^|;\s*)pm_variante=([ABC])(?:;|$)/.exec(document.cookie||'');
  return v ? ({A:'/',B:'/kosten-berechnen',C:'/sofortangebot'})[v[1]] : echt;
}
...
/* Sobald der Kunde selbst etwas ausgelöst hat, hört die Einleitung auf zu
   reden (Martin, 27.08.: Begrüßungssätze fielen mitten in seine Antwort).
...
function beraterChips(){
  /* Stehen alle Angaben und fehlen nur die Kontaktdaten, ist alles andere
     ...
     der Preis ist ja gerechnet. EIN Weg, nicht zwei: das Angebot, die Daten
     stattdessen zu tippen, ist am 29.08. gefallen (siehe `kontakt()`). */
  if(kontaktOffen)
    return setChips([
  ...
  const offen=SCHNELLFRAGEN
    ...
    /* „Was kostet das?" steht direkt über dem Knopf „Preis berechnen" —
       dieselbe Absicht, zweimal angeboten. Das gilt in JEDEM Modus, nicht
       nur im Voll-Chat (Martin, 27.08.: „ich dachte das ist der gleiche
       Code" — war es, aber ich hatte den Filter an den Modus gehängt).
       Die ANZAHL bleibt modusabhängig: im Voll-Chat stehen die Antworten
       als Karten untereinander, dort sprengen drei den iPhone-Schirm. */
    .filter(q=>!/was kostet/i.test(q))
    .slice(0, VOLL?2:3)
...
let abwegig=0;
/* „Für wen suchen Sie?" — die Antworten starten den Fragenlauf und
   beantworten Frage 1 gleich mit: Mutter/Vater = eine Person, Eltern =
...
/* Wie viele Menschen steckt eine Formulierung? Dieselbe Ableitung wie im
   Prompt — hier, damit auch die Chips des Modells den richtigen Wert
   setzen, egal wie es sie genannt hat. Zweierfälle zuerst: „Mutter und
   Vater" ist zwei, obwohl „Mutter" darin steht. */
const PERSONENZAHL=[
  [/ehepaar|zu zweit|\bbeide\b|\b(gro(ß|ss)|schwieger)?eltern\b|(mutter|mama)\s*(und|&|,|\+)\s*(vater|papa)|(vater|papa)\s*(und|&|,|\+)\s*(mutter|mama)|oma\s*(und|&|,|\+)\s*opa|zwei (personen|pflege)/i,'2'],
  [/\b(mutter|vater|mama|papa|oma|opa|gro(ß|ss)(mutter|vater)|tante|onkel|schwiegermutter|schwiegervater|(ehe)?partner(in)?|(ehe)?mann|(ehe)?frau|nachbarin|nachbar|freundin|freund)\b|mich selbst|f(ü|ue)r mich\b/i,'1'],
];
function personenZahl(text){
  for(const [re,v] of PERSONENZAHL) if(re.test(String(text))) return v;
  return null;
}
function personenStart(chip,v,label){
  const s=FLOW[0];
  modus='fragen'; offeneFrage=null;
  protokoll('system','Fragenlauf gestartet',{ereignis:'funnel',grund:'preis'});
  waehle(s,v,label,chip);
}
/* Der Wortlaut darf vom Modell kommen, die Vollständigkeit sichert der Code:
   fehlt einer der drei Fälle (das Modell ließ „Eltern" weg), wird er ergänzt.
   So bleibt der Bot frei zu formulieren, ohne dass eine Auswahl fehlt. */
function fuerWenChips(modellVorschlaege){
  const aus=[]; const abgedeckt=new Set();
  for(const t of (modellVorschlaege||[])){
    const v=personenZahl(t);
    if(!v) continue;                       // „Preis berechnen" u. Ä. gehört nicht hierher
    const art=v==='2'?'zwei':(/vater|papa|opa|onkel|mann\b/i.test(t)?'vater':'mutter');
    if(abgedeckt.has(art)) continue;
    abgedeckt.add(art);
    aus.push({t,f:b=>personenStart(b,v,t)});
  }
  const standard=[
    ['mutter','Für meine Mutter','1'],
    ['vater','Für meinen Vater','1'],
    ['zwei','Für meine Eltern','2'],
  ];
  for(const [art,t,v] of standard)
    if(!abgedeckt.has(art)) aus.push({t,f:b=>personenStart(b,v,t)});
  aus.push({t:'Für jemand anderen',stil:'soft',f:b=>starteFunnel('preis',b,'Für jemand anderen')});
  return aus;
}
...
const FLOW=[
 ...
 {k:'mobil',kurz:'Mobilität',vorschlag:'rollator',warum:'„Mit Rollator“ trifft es bei den meisten ganz gut — nehmen wir das als Ausgangspunkt.',q:'Wie mobil ist die Person, die betreut wird?',
...
/* ─── Kurze Antworten im Kontext der offenen Frage deuten ───────────
   Martin (27.08.): „die Antwort ist doch verständlich — warum fragen wir
   statisch nach und wiederholen dasselbe, statt zu verstehen, dass ‚ich'
   die Antwort darauf ist, und dann nachzufragen, ob Sie mit im Haushalt
   leben?" Genau das: eine Vermutung äußern und bestätigen lassen. Der
   Wert wird NICHT still gesetzt — bestätigt wird per Klick.
   Das ist der Rückfallweg ohne Sprachdienst; mit Modell macht dieses die
   Deutung (Regel im Prompt). */
const DEUTUNG={
  haushalt:[
    [/^\s*(ich|wir|ich selbst|nur ich|auch ich)\b|\bich (wohne|lebe|bin) (mit|da|dort|hier)/i,
     'ja','Sie selbst leben mit im Haushalt'],
    [/\b(mein|meine|unser|unsere)\s+(mann|frau|partner|partnerin|tochter|sohn|kind|kinder|schwester|bruder|enkel|familie|eltern)/i,
     'ja','es lebt noch jemand mit im Haushalt'],
    [/\b(niemand|keiner|allein|alleine|nur sie|nur er|ganz allein)\b/i,
     'nein','die zu betreuende Person lebt allein'],
  ],
  nacht:[
    [/\b(nie|nichts|nix|schl(ä|ae)ft durch|durchschlafen|ruhig)\b/i,'nein','nachts ist keine Hilfe nötig'],
    [/\b(manchmal|selten|ab und zu|hin und wieder|gelegentlich)\b/i,'gelegentlich','nachts ist gelegentlich Hilfe nötig'],
    [/\b(immer|jede nacht|t(ä|ae)glich|st(ä|ae)ndig|oft|(ö|oe)fter|mehrmals|mehrfach)\b/i,'mehrmals','nachts ist regelmäßig mehrfach Hilfe nötig'],
    [/\b(toilette|wc|windel|umlagern|unruhig|aufsteh)/i,'taeglich','nachts ist jede Nacht Hilfe nötig'],
  ],
  mobil:[
    [/\b(l(ä|ae)uft|geht|selbst(st(ä|ae)ndig)?|allein|mobil|noch fit|treppen)\b/i,'mobil','die Person geht noch selbstständig'],
    [/\b(rollator|gehhilfe|gehwagen|stock)\b/i,'rollator','ein Rollator ist im Einsatz'],
    [/\b(rollstuhl|rolli|sitzt im)\b/i,'rollstuhl','ein Rollstuhl ist nötig'],
    [/\b(liegt|bett|bettl(ä|ae)gerig|steht nicht|kann nicht aufstehen)\b/i,'bett','die Person ist bettlägerig'],
  ],
  fuehrerschein:[
    [/\b(ja|gern|w(ä|ae)re gut|einkauf|arzt|fahren|auto|termine)\b/i,'ja','die Pflegekraft soll Auto fahren können'],
    [/\b(nein|nicht n(ö|oe)tig|brauchen wir nicht|kein auto|taxi|zu fu(ß|ss))\b/i,'nein','ein Führerschein ist nicht nötig'],
  ],
  geschlecht:[
    [/\b(egal|beides|spielt keine rolle|unwichtig)\b/i,'egal','das Geschlecht ist Ihnen egal'],
    [/\b(frau|weiblich|dame|lieber eine)\b/i,'weiblich','Sie möchten lieber eine Frau'],
    [/\b(mann|m(ä|ae)nnlich|herr|lieber einen)\b/i,'maennlich','Sie möchten lieber einen Mann'],
  ],
  deutsch:[
    [/\b(wenig|kaum|paar w(ö|oe)rter|grundlagen|einfach reicht|nicht so wichtig)\b/i,'grundlegend','einfache Verständigung reicht Ihnen'],
    [/\b(unterhalten|reden|sprechen k(ö|oe)nnen|verstehen|alltag)\b/i,'kommunikativ','die Kraft soll sich im Alltag unterhalten können'],
    [/\b(sehr gut|flie(ß|ss)end|perfekt|richtig gut|wichtig)\b/i,'gut','gutes Deutsch ist Ihnen wichtig'],
  ],
  personen:[
    [/^\s*(ich|mich|f(ü|ue)r mich)\b/i,'1','Sie suchen die Betreuung für sich selbst'],
  ],
};
function deuten(s,text){
  for(const [re,v,satz] of (DEUTUNG[s.k]||[]))
...
async function matching(){
  ...
  /* Zwei Abschlüsse, weil die Protokolle einen klaren Abbruchpunkt zeigen:
     von zwölf Sitzungen, die den Fragenlauf durchhatten, brachen elf auf der
     Kontaktabfrage ab (27.-29.08.). Der alte Text sagte erst „Fertig, Ihr Preis
     ist berechnet" und nahm den Preis im nächsten Satz wieder weg — samt
     Begründung, die die Schranke erst recht sichtbar machte. Das Formular des
     Kostenrechners verlangt DIESELBEN drei Pflichtfelder, fragt aber nur nach
     der Lieferadresse („Wohin dürfen wir Ihr Angebot senden?") und konvertiert
     deutlich besser. Der Float übernimmt diese Rahmung; der Voll-Chat behält
     den alten Wortlaut, bis die Seite wieder Verkehr bekommt (Martin, 30.08.:
     „die voll bleibt erstmal raus") — sonst vermischen sich die Messungen. */
  if(VOLL){
    ...
    // Die Häkchen-Animation darüber endet bereits mit „Ihr Sofortangebot steht" —
    // die gute Nachricht ist gesagt. Hier nur noch, WAS gefunden wurde, und die
    // Lieferfrage. Kein Preis-Versprechen, das der nächste Satz zurücknimmt.
    await sagen('<b>5 passende Pflegekräfte</b> habe ich für Sie gefunden.');
    await pause(ruhig?0:420);
    await sagen('Wohin darf ich Ihr Angebot schicken?',
                'Preis und Profile sehen Sie direkt im Anschluss — die E-Mail ist nur Ihre Kopie zum Nachlesen. Kostenlos, unverbindlich, kein Vertrag vor Ihrer Auswahl.');
  }
  kontakt();
}
...
function kontakt(){
  ...
  /* Kein zweiter Weg mehr (Martin, 29.08.: „warum bieten wir überhaupt die
     Kontaktdaten im Chat einzutragen — ist doch unnötig"). Die drei Felder
     stehen jetzt direkt über der Eingabezeile; ein Angebot, dasselbe
     stattdessen zu tippen, ist eine Abzweigung neben dem Ziel — und war der
     laengere, fehleranfaelligere Weg: erst tippen, dann vom Modell auslesen
     lassen, dann bestaetigen. Wer trotzdem tippt, wird weiter verstanden
     (typ=kontakt bleibt) — angeboten wird es nicht.
     Alte Vorschlaege muessen trotzdem weg, sonst stehen Wissensfragen unter
     dem Formular. */
  setChips([]);
  ...
  go.onclick=async()=>{
    ...
    if(leadId){
      ...
      if(typeof window.oaiq==='function') window.oaiq('measure','lead_created',{type:'customer_action',amount:2000,currency:'EUR'},{event_id:leadId}); // amount in Cent (Doku: 4200 = 42,00), synchron zu lib/oaiq.ts WERT_ANFRAGE_MINOR
    }
...
const THEMEN=[
 ...
 /* Wer betreut wird, sagt die Personenzahl mit (Martin, 27.08.) — dieselbe
    Ableitung wie im Prompt, hier für den Fall, dass der Sprachdienst
    ausfällt. Passen BEIDE Stufen („meine Mutter und mein Vater"), fragt
    der Chat kurz nach, statt zu raten. */
 {k:'personen',
  w:/ehepaar|zu zweit|beide|eltern|mutter|vater|mama|papa|oma|opa|gro(ß|ss)(mutter|vater)|tante|onkel|(ehe)?partner|zwei (personen|pflege)|mich selbst|f(ü|ue)r mich/i,
  stufen:[['1',/\b(mutter|vater|mama|papa|oma|opa|gro(ß|ss)(mutter|vater)|tante|onkel|schwiegermutter|schwiegervater|(ehe)?partner(in)?|nachbarin|nachbar)\b|mich selbst|f(ü|ue)r mich\b/i],
          ['2',/ehepaar|zu zweit|\bbeide\b|\b(gro(ß|ss)|schwieger)?eltern\b|(mutter|mama)\s*(und|&|,)\s*(vater|papa)|(vater|papa)\s*(und|&|,)\s*(mutter|mama)|oma\s*(und|&|,)\s*opa|zwei (personen|pflege)/i]],
  grund:'Ich trage zwei pflegebedürftige Personen ein — sagen Sie einfach Bescheid, wenn es doch nur eine ist.'},
...
/* Wie die Notiz klingt (Martin, 27.08.): „Personen: Für meine Eltern" war
   Formularsprache — der Mensch sagt „Ich notiere: 2 Personen." Für die
   Felder mit unhandlichen Optionstexten steht die kurze Fassung hier. */
const NOTIZ={
  personen:{'1':'1 pflegebedürftige Person','2':'2 pflegebedürftige Personen'},
  haushalt:{ja:'weitere Personen im Haushalt',nein:'niemand sonst im Haushalt'},
  pflegegrad:{'0':'noch kein Pflegegrad','1':'Pflegegrad 1','2':'Pflegegrad 2',
              '3':'Pflegegrad 3','4':'Pflegegrad 4','5':'Pflegegrad 5','?':'Pflegegrad noch offen'},
  mobil:{mobil:'geht selbstständig',rollator:'mit Rollator',
         rollstuhl:'im Rollstuhl',bett:'bettlägerig'},
  nacht:{nein:'nachts keine Hilfe nötig',gelegentlich:'nachts gelegentlich Hilfe',
         taeglich:'jede Nacht Hilfe',mehrmals:'jede Nacht mehrfach Hilfe'},
  fuehrerschein:{ja:'mit Führerschein',nein:'Führerschein nicht nötig'},
  geschlecht:{egal:'Geschlecht egal',weiblich:'weibliche Pflegekraft',maennlich:'männliche Pflegekraft'},
  deutsch:{grundlegend:'Deutsch grundlegend',kommunikativ:'Deutsch kommunikativ',gut:'Deutsch gut'},
};
function notizText(s,v,label){
  const k=(NOTIZ[s.k]||{})[v];
  return k || ((s.kurz||s.q)+': '+label);
}
/* Mehrere Angaben aus EINER Nachricht (Martin, 27.08.: „mehr Freiheit für
   den Bot"). Das Modell liefert alles, was im Satz steckt; der Code trägt
   es ein, bestätigt es in EINEM Satz und geht zur nächsten OFFENEN Frage —
   niemals zu einer, die der Kunde gerade beantwortet hat. */
async function mehrfachUebernehmen(liste){
  chips.innerHTML='';
  kundeAktiv=true;
  const gesetzt=[]; let unklar=null;
  for(const e of liste){
    const s=FLOW.find(f=>f.k===e.feld);
    if(!s || !e.werte || !e.werte.length) continue;
    // Mehrdeutig (gestuftes Feld, zwei Situationen) — nach den eindeutigen klären.
    if(e.werte.length>1){ if(!unklar) unklar={s,werte:e.werte}; continue; }
    const v=e.werte[0], label=(s.o.find(x=>x[0]===v)||[,v])[1];
    const geaendert=antwort[s.k]!==undefined && antwort[s.k]!==v;
    antwort[s.k]=v;
    gesetzt.push({s,v,label,geaendert});
    if(offeneFrage && offeneFrage.k===s.k) offeneFrage=null;
  }
  if(gesetzt.length===1){
    const g=gesetzt[0];
    await sagen((g.geaendert?'Danke, gut dass Sie das sagen — ich ändere das auf ':'Ich notiere: ')+
                '<b>'+notizText(g.s,g.v,g.label)+'</b>');
  }else if(gesetzt.length>1){
    // Ein Satz für alles — acht Quittungen hintereinander wären Formular-Ping-Pong.
    await sagen('Ich notiere: '+gesetzt.map(g=>'<b>'+notizText(g.s,g.v,g.label)+'</b>').join(' · '));
  }
  fortschritt();
  if(unklar) return mehrdeutig(unklar.s,unklar.werte,(v,l)=>uebernehmen(unklar.s,v,l));
  return weiterNachAngabe();
}
/* Wohin nach einer Übernahme — EINE Stelle, damit der Weiterlauf nicht
   wieder pro Pfad auseinanderläuft (Bug 27.08.). */
async function weiterNachAngabe(){
  if(offeneFrage){
    await pause(ruhig?0:380);
  ...
  /* HARTE REGEL: Wer eine Preisfrage beantwortet hat, ist im Fragenlauf —
     ...
     nächste Frage, nie das Menü. naechste() rechnet von selbst, wenn
...
async function uebernehmen(s,v,label){
  ...
  antwort[s.k]=v;
  await sagen((schonDa?'Danke, gut dass Sie das sagen — ich ändere das auf ':'Ich notiere: ')+
              '<b>'+notizText(s,v,label)+'</b>');
  if(offeneFrage && offeneFrage.k===s.k){ offeneFrage=null; }
  fortschritt();
  return weiterNachAngabe();
}
...
async function verarbeite(r,kundentext){
  ...
  if((r.typ==='antwort'||r.typ==='nachtrag') && (r.feld || (r.felder&&r.felder.length))){
    // Alles, was in der Nachricht steckt — das Einzelfeld ist darin enthalten
    // (der Server legt es hinein), ältere Antworten ohne `felder` ebenso.
    const alle = (r.felder && r.felder.length)
      ? r.felder
      : (r.feld && r.werte.length ? [{feld:r.feld,werte:r.werte}] : []);
    const gueltig = alle.filter(e=>FLOW.some(f=>f.k===e.feld) && e.werte && e.werte.length);
    if(gueltig.length){
      ...
      if(r.text){ chips.innerHTML='';
        sagenJetzt(r.text);
        await pause(ruhig?0:420); }
      // Genau EINE Angabe zur offenen Frage: der eingespielte Weg mit
      // Reaktionssatz („Gute Wahl, das reicht für den Alltag").
      if(gueltig.length===1){
        const s=FLOW.find(f=>f.k===gueltig[0].feld), werte=gueltig[0].werte;
        const offen = offeneFrage && offeneFrage.k===s.k;
        if(werte.length>1)
          return mehrdeutig(s,werte, offen ? (v,l,b)=>waehle(s,v,l,b||null)
                                           : (v,l)=>uebernehmen(s,v,l));
        const v=werte[0], label=(s.o.find(x=>x[0]===v)||[,v])[1];
        return offen ? waehle(s,v,label,null,true) : uebernehmen(s,v,label);
      }
      return mehrfachUebernehmen(gueltig);
    }
  }
  ...
  if(r.typ==='kontakt' && kontaktOffen){
    ...
    const knopf=W.getElementById('abs');
    if(!gesetzt){
      /* Nicht noch einmal tippen lassen: die Felder stehen direkt über der
         Eingabezeile, und beim zweiten Versuch scheitert das Auslesen
         erfahrungsgemäß aus demselben Grund wie beim ersten. */
      await sagen(r.text||'Das habe ich nicht sicher herauslesen können — tragen Sie '+
                  '<b>Name, E-Mail und Telefon</b> bitte direkt in die Felder darüber ein.');
      return beraterChips();
    }
    zumKontakt();
  ...
  if(r.typ==='abwegig') return setChips(fuerWenChips(r.chips));
  ...
  if(!offeneFrage && antwort.personen===undefined && fuerWenGemeint(r.text,r.chips))
    return setChips(fuerWenChips(r.chips));
...
async function frageLokal(text){
  ...
  /* Steht eine Frage offen, wollte der Kunde ANTWORTEN — „ich" auf „Leben
     weitere Personen mit im Haushalt?" ist keine Wissensfrage, sondern
  ...
  if(!e && offeneFrage){
    ...
    /* Nichts zu deuten: beim ERSTEN Mal nachfragen, beim zweiten Mal nicht
       ...
       Vorschlag anbieten bzw. um einen Tipp bitten. */
    nachgefragt[s.k]=(nachgefragt[s.k]||0)+1;
    if(nachgefragt[s.k]===1){
      await sagen('Das habe ich nicht sicher verstanden — meinen Sie eine dieser Antworten '+
                  'auf <b>'+s.q+'</b>?');
    }else if(s.vorschlag){
      return vorschlagen(s);
    }else{
      await sagen('Ich komme da gerade nicht mit — tippen Sie einfach auf die Antwort, '+
                  'die am ehesten passt. Alles andere klären wir später in Ruhe.');
    }
    return optionenAnbieten([
...
function pillePruefen(){
  ...
  /* Eine Bildschirmhoehe je Frage. Vorher 1,5 — da wechselte sie bei
     normalem Lesen kaum sichtbar (Martin, 30.08.: „wenn ich scrolle,
     aendern sich die fragen nicht"). */
  pilleSetzen(Math.floor(Math.max(0,basis)/innerHeight));
}
addEventListener('scroll',pillePruefen,{passive:true});

function versteckePille(){
  pille.classList.remove('on');
  /* Zaehler zuruecksetzen: sonst steigt pilleSetzen beim Wiederkommen mit
     demselben Index frueh aus und die Pille bliebe unsichtbar. */
  frageNr=-1;
}
  ...
  h.innerHTML=
    '<div class="oben">'+
      ...
      '<div class="werblock">'+
        ...
        '<p class="rolle">Ihre digitale KI-Pflegeberaterin</p>'+
      '</div>'+
...
/* Der Kunde war schneller als die Einleitung (Martin, 27.08.): Wer sofort
   „Eltern" tippt, bekam die Begrüßungssätze MITTEN in die Übernahme
   geschoben — Pria redete an ihm vorbei. Sobald der Fragenlauf begonnen
   hat oder eine Frage offen ist, bricht der Einstieg still ab. */
const ueberholt=()=>kundeAktiv||modus==='fragen'||!!offeneFrage||kontaktOffen||uebergeben;
async function oeffne(start){
  ...
  if(start==='lp'){
    /* Landingpage /sofortangebot: OFFENER Einstieg (Martin, 26.08. nachts) —
...
/* Die Seite rendert den Abschnitt ZWEIMAL — einmal in `lg:hidden` fuers
   Handy, einmal in `hidden lg:block` fuer den Desktop — und beide tragen
   dieselbe id. `getElementById` liefert die ERSTE, und das ist auf dem
   Desktop die unsichtbare mit Hoehe 0 an Position 0. Folge (Martin, 30.08.,
   Screenshot): Pria stand sofort im Hero, weil „Anker erreicht" ab dem
   ersten Pixel galt. Auf dem Handy fiel es nicht auf, weil dort zufaellig
   die sichtbare zuerst kommt.
   Deshalb: die SICHTBARE nehmen, und bei jedem Aufruf neu — beim Drehen
   des Geraets wechselt sie. */
const ANKER_ID='ablauf';
function ankerEl(){
  const alle=SEITE.querySelectorAll('#'+ANKER_ID);
  for(const e of alle){ if(e.getBoundingClientRect().height>0) return e; }
  return alle[0]||null;
}
const anker=ankerEl();
...
/* ─── Voll-Chat: sofort offen, als Seite ────────────────────────────
   Auf /sofortangebot ist der Chat kein Gast, sondern der Gastgeber: keine
