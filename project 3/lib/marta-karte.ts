/**
 * Martas Signaturkarte — EINE Vorlage für alle Mails mit Karte.
 *
 * Bis 17.09.2026 stand die Karte als Kopie in sieben Mail-Vorlagen
 ...
 *
 * Handy-Verhalten aus #702 (11.09.2026) bleibt: Name/Zeiten dürfen
 * umbrechen, Siegel-Spalte per .sig-siegel-* schmaler, Logos schrumpfen über
 * max-width. Jede Mail-Shell mit Karte bindet MARTA_KARTE_MOBIL_CSS in ihre
 * Media-Query ≤480 px ein.
 *
 * Vermittler-Mails (fuer: 'vermittler') bekommen dieselbe Karte OHNE Sterne
 * und mit dem alten dritten Fakt: „Keine Vermittlungsgebühr" neben einem
 * Provisionsblock wäre ein Widerspruch (vermittler.ts, Registry #59).
 *
 * Martas Foto kommt immer von primundus.de: das steht unabhängig vom
 * Kostenrechner-Deploy (Begründung in buildMartaSig, 20.08.2026). Siegel und
 * Presselogos kommen vom übergebenen siteUrl.
 *
 * ⚠️ KOPIE in supabase/functions/send-scheduled-emails/martaKarte.ts —
 * src/__tests__/martaKarte.test.ts prüft, dass beide Zeichen für Zeichen
 * dieselbe Karte liefern. Änderungen IMMER in beiden Dateien.
 */
import { type BewertungsStand, ERFAHRUNGEN_URL } from './bewertungen-stand';
